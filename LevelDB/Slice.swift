...
extension Slice {
    ...
    public func asData(copy: Bool = true) -> Data {
        if copy {
            return Data(bytes: UnsafePointer<UInt8>(OpaquePointer(UnsafeRawPointer(self.bytes))), count: self.length)
        } else {
...
extension String: KeyType {
    public init(bytes: UnsafeRawPointer, count: Int) {
        ...
        data.assign(from: UnsafePointer<Int8>(OpaquePointer(bytes)), count: count)
    ...
    public func asData() -> Data {
        return self.withCString { p in
            var i = 0
            while (p + i).pointee != 0 { i += 1 }
            return Data(bytes: UnsafePointer<UInt8>(OpaquePointer(p)), count: i)
        }
    }
}
