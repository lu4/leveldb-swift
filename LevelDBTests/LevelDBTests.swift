...
class LevelDBTests: XCTestCase {
    ...
    func createDb(_ options : Options = Options(createIfMissing: true)) -> Database? {
        #if TARGET_OS_IPHONE
    ...
    func testPut() {
        ...
        let response = try! db.get("test").map({ NSString(data: $0, encoding: String.Encoding.utf8) as! String })
    ...
    func testWriteBatch() {
        ...
        try! db.write(batch);
        let response = try! db.get(key2).map({ NSString(data: $0, encoding: String.Encoding.utf8) as! String })
    ...
    func testKeyValueSequence() {
        ...
        for (key, value) in db.values() {
            index++
            let keyName = "test\(index)"
            let valueString = NSString(data: value!, encoding: String.Encoding.utf8) as! String
